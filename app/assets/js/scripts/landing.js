    ...
    sysAEx.on('message', (m) => {
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    'Aucune Installation Java<br>Compatible trouvée',
                    `Pour rejoindre les serveurs, vous avez besoin d'une installation 64 bits de Java ${javaVer}. Souhaitez-vous que nous installions une copie ?`,
                    'Installer Java',
...
function dlAsync(login = true){
    ...
    aEx.on('error', (err) => {
        ...
        showLaunchFailure('Erreur lors du lancement', err.message || 'Veuillez vérifier la console (CTRL + Maj + i) pour plus de détails.')
    })
    aEx.on('close', (code, signal) => {
        if(code !== 0){
            loggerLaunchSuite.error(`AssetExec a quitté avec le code ${code}, hypothèse d'erreur.`)
        ...
        if(m.context === 'validate'){
            switch(m.data){
                ...
                case 'assets':
                    ...
                    loggerLaunchSuite.info('Validation de l\'actif terminée')
                ...
                case 'files':
                    ...
                    loggerLaunchSuite.info('Validation du fichier terminée.')
            ...
            switch(m.data){
                case 'download':
                    ...
                    if(m.error.code === 'ENOENT'){
                        ...
                        showLaunchFailure(
                            'Erreur de téléchargement',
                            'Veuillez vérifier la console (CTRL + Maj + i) pour plus de détails. Veuillez réessayer.'
                        )
                    }
    ...
    refreshDistributionIndex(true, (data) => {
        ...
        refreshDistributionIndex(false, (data) => {
            ...
            if(DistroManager.getDistribution() == null){
                showLaunchFailure('Erreur fatale', 'Impossible de charger une copie de l\'index de distribution. Veuillez vérifier la console (CTRL + Maj + i) pour plus de détails.')
